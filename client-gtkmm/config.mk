...
GTKMM_CPPFLAGS ?= $(shell pkg-config --cflags gtkmm-3.0)
...
RCKAM_CLIENT_DIR?=$(RCKAM_ROOT_DIR)/client-gtkmm
#RCKAM_CLIENT_DIR?=$(RCKAM_MAKEFILE_DIR)
RCKAM_COMMON_DIR?=$(RCKAM_ROOT_DIR)/common
RCKAM_MAKE_DIR?=$(RCKAM_CLIENT_DIR)
RCKAM_TEST_DIR?=$(RCKAM_ROOT_DIR)/tests
RCKAM_BUILD_DIR_BASE?=$(RCKAM_CLIENT_DIR)/build
...
CPPFLAGS += $(GTKMM_CPPFLAGS)
CPPFLAGS += -I $(RCKAM_CLIENT_DIR)
CPPFLAGS += -I $(RCKAM_ROOT_DIR)
CXXFLAGS?=-std=c++17
CFLAGS?=-std=c99 

LDFLAGS?=
LDFLAGS += $(GTKMM_LDFLAGS)
...
sources := $(wildcard $(RCKAM_CLIENT_DIR)/*.cpp)
#programs := $(sources:$(RCKAM_CLIENT_DIR)/%.cpp=%)
programs := rckam-client
all_lib_sources := $(filter-out $(programs:%=$(RCKAM_CLIENT_DIR)/%.cpp), $(sources))
all_lib_objects := $(all_lib_sources:$(RCKAM_CLIENT_DIR)/%.cpp=$(BUILD)/%.o)
