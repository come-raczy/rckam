...
Options::Action Options::parse(int argc, const char * const argv[])
{
  try
  {
    ...
    if (vm_.count("response-file"))
    {
       ...
       if (!ifs)
       {
         std::clog << "Could not open response file: " << vm_["response-file"].as<std::string>().c_str() << std::endl;
         return ABORT;
       }
       ...
       if (!ifs)
       {
         std::clog << "Could not read response file: " << vm_["response-file"].as<std::string>().c_str() << std::endl;
         return ABORT;
       }
...
std::string Options::help(const OptionDescriptionPtrs &sortedOptions, const bool markdown) const
{
  ...
  if (!markdown)
  {
    ...
    bpo::options_description printedDescriptions("Command line options",
