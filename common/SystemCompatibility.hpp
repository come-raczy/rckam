...
#if defined(_WIN32) && !defined(RCKAM_CYGWIN)
...
#define RCKAM_TRACE_STAT(prefix)
...
#define RCKAM_TRACE_STAT(prefix) {\
    ...
    RCKAM_THREAD_CERR << "STAT: " << prefix << " " << rckam::common::parseStat(statm) << std::endl;\
    }
 ...
 *        not mess up reopening for append. Notice that posix_fallocate does not do the job.
 */
